...
[`DocumentationExamplesTests.cs`](tests/DocumentationExamplesTests.cs), including the injection
payloads in Example 7 — so the documentation cannot quietly drift from the behaviour. A separate
[legacy-parity suite](tests/LegacyParityTests.cs), whose expected values were captured by running
...
[DESIGN.md](DESIGN.md) covers the architecture and the alternatives that were rejected.
[SUCCESSOR-NOTES.md](SUCCESSOR-NOTES.md) records how a future generation should be approached.
